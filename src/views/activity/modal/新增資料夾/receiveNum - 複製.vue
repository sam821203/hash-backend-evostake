    ...
    <div class="table" :style="{ height: receiveNumDetailData?.player_bonus_events ? '500px' : '115px' }">
      ...
      >
        ...
        <el-table-column :label="$t('領取狀態')" align="center">
          <template #default="scope">
            <span v-if="scope.row.status===0">{{ $t('未領取') }}</span>
            <span v-if="scope.row.status===1">{{ $t('已領取') }}</span>
        ...
        <el-table-column :label="$t('狀態變更時間')" align="center">
          <template #default="scope">
            <span v-time="scope.row.last_change_time"></span>
          </template>
        </el-table-column>
...
<script setup>
import { toRefs, computed, ref, nextTick } from 'vue-demi'
...
const { receiveNumModal } = toRefs(props)
